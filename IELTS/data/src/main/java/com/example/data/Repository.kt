...
class Repository : RepositoryInterface {
    ...
    private val _dashboardItemsFlow = MutableStateFlow<Map<DashboardCategory, List<DashboardItems>>>(emptyMap())
    override val dashboardItemsFlow: StateFlow<Map<DashboardCategory, List<DashboardItems>>> = _dashboardItemsFlow
     ...
     */
    private fun createDashboardItemsForCategory(category: DashboardCategory, query: String): List<DashboardItems> {
        return listOf(
            DashboardItems(
                category.iconUri,
                category.title,
    ...
    private fun fallbackDashboardItems(category: DashboardCategory): List<DashboardItems> {
        return listOf(
            DashboardItems(
                ...
                "Lesson",
                category.color,
                YouTubeLink.getLink("${category.title} IELTS Lesson")
            )
        )
    }
}
