...
class GroqProvider : APIProvider {
    ...
    override fun getApiKey(): String {
        return try {
            BuildConfig.GROQ_API_KEY.also { key ->
                if (key.isBlank()) {
                    Log.e(TAG, "Groq API key is missing or empty")
                } else {
                    Log.d(TAG, "Groq API key loaded successfully")
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error getting Groq API key from BuildConfig", e)
    ...
    override suspend fun getChatCompletion(completion: ChatCompletion): ResponseBody {
        return createRetrofit().create(ChatApi::class.java).getChatCompletion(completion)
    }
} 